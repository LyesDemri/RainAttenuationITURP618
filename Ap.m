...
thetadeg=block.DialogPrm(2).Data;%degrés
...
LG=Ls*cos(thetarad);
%étape4
R001=block.DialogPrm(7).Data;
%étape5
tau=block.DialogPrm(8).Data;
kH=block.DialogPrm(9).Data;
alphaH=block.DialogPrm(10).Data;
kV=block.DialogPrm(11).Data;
alphaV=block.DialogPrm(12).Data;
k=(kH+kV+(kH-kV)*(cos(thetarad))^2*cos(2*tau))/2
alpha=(kH*alphaH+kV*alphaV+(kH*alphaH-kV*alphaV)*(cos(thetarad))^2*cos(2*tau))/2*k
gammaR=k*(R001)^(alpha)
%étape6
r001=1/(1+0.78*sqrt((LG*gammaR)/f)-0.38*(1-exp(-2*LG)))
%étape7
zeta=atan((hR-hs)/(LG*r001))
if zeta>thetadeg 
...
block.OutputPort(1).Data = block.InputPort(1).Data*Ap;
